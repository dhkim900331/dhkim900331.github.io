...
let rootHandle, postHandle, normalizing = false, lastImageUrl = '';
...
async function saveCurrent() { if(!postHandle) throw new Error('새 글을 만들거나 기존 글을 먼저 여세요.'); const w=await postHandle.createWritable(); await w.write(source.value); await w.close(); }
document.querySelector('#save').onclick=async()=>{ try { await saveCurrent(); status.textContent='저장했습니다.'; } catch(e) { status.textContent=e.message; } };
async function importImage(file) { try { if(!rootHandle) throw new Error('먼저 블로그 저장소를 선택하세요.'); const postSlug=currentSlug(); let target=await directory(rootHandle,'assets'); target=await directory(target,'posts'); target=await directory(target,'images'); target=await directory(target,category.value); target=await directory(target,postSlug); const safe=file.name.replace(/[^A-Za-z0-9._-]/g,'-'); let output=await target.getFileHandle(safe, {create:true}); if((await output.getFile()).size > 0) output=await target.getFileHandle(`${Date.now()}-${safe}`, {create:true}); const writable=await output.createWritable(); await writable.write(file); await writable.close(); const url=`/assets/posts/images/${category.value}/${postSlug}/${output.name}`; lastImageUrl=url; const at=source.selectionStart; source.setRangeText(`![${output.name}](${url})`,at,source.selectionEnd,'end'); source.dispatchEvent(new Event('input')); status.textContent=`이미지를 복사하고 사이트 절대 경로 URL을 삽입했습니다: ${url}`; } catch(e) { status.textContent=e.message; } }
document.querySelector('#copyImageUrl').onclick=async()=>{try{if(!lastImageUrl) throw new Error('먼저 이미지를 추가하세요.'); await navigator.clipboard.writeText(lastImageUrl); status.textContent=`클립보드에 복사했습니다: ${lastImageUrl}`;}catch(e){status.textContent=e.message;}};
document.querySelector('#setToken').onclick=async()=>{try{const token=prompt('GitHub Personal Access Token을 입력하세요. 토큰은 Windows Git Credential Manager에만 저장되며 이 저장소에는 저장되지 않습니다.'); if(!token) return; const response=await fetch('/api/token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})}); const result=await response.json(); if(!result.ok) throw new Error(result.message); status.textContent=result.message;}catch(e){status.textContent=e.message;}};
document.querySelector('#publish').onclick=async()=>{try{await saveCurrent(); const message=prompt('커밋 메시지', 'Publish blog update'); if(message===null) return; status.textContent='전체 블로그 감사 후 GitHub에 게시하고 있습니다…'; const response=await fetch('/api/publish',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message})}); const result=await response.json(); if(!result.ok) throw new Error(`${result.stage}:\n${result.output || result.message}`); status.textContent=`게시 완료.\n${result.output}`;}catch(e){status.textContent=e.message;}};
